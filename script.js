(function () {
    ...
    try {
      ...
      categories.forEach((cat) => {
        const id =
          ...
          cat.category_id;
        const name =
          cat.name || cat.category_name || cat.category || cat.category_name;
  ...
  function renderCards(plants) {
    if (!plants || plants.length === 0) {
      treeCardsContainer.innerHTML = `<p style="text-align:center; padding:20px;">No Plants found.</p>`;
      return;
    }
    ...
    plants.forEach((plant) => {
      const id =
        ...
        plant.id;
      const name =
        plant.name || plant.plant_name || plant.common_name || "Unknown Plant";
      const image =
        plant.image ||
        plant.picture ||
        (plant.image && plant.images[0]) ||
        "https://i.ibb.co/7RWyzfX/tree1.jpg";
      const description =
        plant.description ||
        plant.short_description ||
        "No Description available.";
      const category =
        plant.category || plant.category_name || plant.type || "General";
      const price =
        plant.price ||
        plant.rate ||
        plant.cost ||
      ...
      <h4 class="tree-name" data-id="${id}" style="cursor:pointer;color:#0b6b3a;">
        ${escapeHtml(name)}
      </h4>
  ...
  async function showPlantDetails(id) {
    try {
      ...
      if (!plant) {
        ...
        const data = await res.json();
        plant = data.data || data.plant || data || null;
      ...
      const name =
        plant.name || plant.plant_name || plant.common_name || "Plant";
      const image =
        plant.image ||
        plant.img ||
        plant.picture ||
  ...
  function renderCart() {
    ...
    cart.forEach((ci) => {
      ...
      itemDiv.innerHTML = `
                    <div>
                    <h4 style="margin:0; font-size:14px; color:#0b6b3a;">${escapeHtml(
                      ci.name
                    )}</h4>
                    ...
                    <button class="remove-item" data-id="${
                      ci.id
                    }" style="color:white; border:none; padding:6px 8px; border-radius:6px; cursor:pointer;">❌</button>
                    </div>
                `;
